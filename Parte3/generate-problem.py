...
def main():
    ...
    parser.add_option('--transporter-capacity', metavar='NUM', type=int, dest='transporter_capacity',
                      default=4, help='maximum number of crates that fit in a transporter')
    ...
    if options.transporter_capacity < 1:
        print("Transporter capacity must be >= 1")
        sys.exit(1)
    ...
    print("Transporter cap\t", options.transporter_capacity)
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        for i in range(len(crates_with_contents)):
            for c in crates_with_contents[i]:
                f.write("\t(has-content " + c + " " + content_types[i] + ")\n")

        for i in range(len(person)):
            loc = random.choice(location[1:])
            f.write("\t(at-person " + person[i] + " " + loc + ")\n")
            f.write("\t(person-free " + person[i] + ")\n")

        for i in range(len(location)):
            for j in range(len(location)):
                cost = 0 if i == j else flight_cost(location_coords, i, j)
        ...
        for x in range(options.persons):
            for y in range(len(content_types)):
                if need[x][y]:
                    f.write("\t(has " + person[x] + " " + content_types[y] + ")\n")
