...
PLANNERS = {
    "metric-ff": ["planutils", "run", "metric-ff"],
    "lama-first": ["planutils", "run", "downward", "--", "--alias", "lama-first", "--overall-time-limit", "60"],
    "seq-sat-fdss-2": ["planutils", "run", "downward", "--", "--alias", "seq-sat-fdss-2", "--overall-time-limit", "60"],
    "seq-sat-fd-autotune-2": ["planutils", "run", "downward", "--", "--alias", "seq-sat-fd-autotune-2", "--overall-time-limit", "60"],
}


re_cost = re.compile(r"(?:Plan cost|Metric|cost)\s*[:=]?\s*([0-9]+)", re.IGNORECASE)
...
def run(problem: str, planner_name: str, base_cmd: list[str]):
    ...
    try:
        ...
        # Consideramos resuelto si hay coste o longitud de plan
        if cost_m or len_m:
...
def main():
    ...
    with open("punto9_satisficing.csv", "w", newline="") as f:
        w = csv.DictWriter(
            f,
